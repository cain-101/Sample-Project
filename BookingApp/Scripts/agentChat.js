...
$(function () {
    ...
    myHub.client.newChat = function (id) {
        var snd = new Audio('/ChatBot/notif/newchat.mp3');
        snd.play();

        var d = new Date();

        var session = [];
        session.push('<div class="sticky-top p-1 text-center bg-light" style="height:40px;width:100%;color:gray">Chat started at ' + d.getHours() + ':' + d.getMinutes()+'</div>');
        ...
        $('#chat-sessions').prepend(
            ...
            '<p style="cursor:pointer">New message(s) <span class="badge badge-warning">0</span></p></div>' +
            '</div>');
    ...
    myHub.client.addMessage = function (id, from, value) {
        if (id == 'internal') {
            ...
            if (from == "Visitor") {
                msgesBody += "<p class='text-right mb-0 mt-0'><span style='border-radius:50px;background-color:rgb(57, 123, 230);padding:5px;color:white'><b>" + from + "</b></span></p>\
                               <p class='p-3 text-right mr-4 mt-0 mb-0' style='color:rgb(57, 123, 230)'>"+ value + "</p>";
            }
            else {
                msgesBody += "<p class='text-left pl-0 ml-0 mb-0 mt-0'><span style='border-radius:50px;background-color:rgb(207, 113, 72);padding:5px;color:white'><b>" + from + "</b></span></p>\
                             <p class='p-3 text-left ml-3 mt-0 mb-0' style='color:rgb(207, 113, 72)'>"+ value + "</p>";
            }
            msgesBody += "</div>";
            $('#chatmsgsinternal').append(msgesBody);
            ...
            $('#chatmsgs' + id).append(msgesBody);

            scrollDiv($('#chatmsgs' + id));
        } else {
            var snd = new Audio('/ChatBot/notif/newmsg.mp3');
            snd.play();
            ...
            chatMessages[id].push(msgesBody);
